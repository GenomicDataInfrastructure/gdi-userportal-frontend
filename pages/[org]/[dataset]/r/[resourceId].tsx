...
import { ResourceInfo } from "@portaljs/ckan";
// import {
//   getResourcesWithAliasList,
//   getDatastoreResourceInfo,
//   getResourceMetadata,
// } from "@portaljs/ckan";
import { CKAN } from "@portaljs/ckan"
import { convertFieldSchema } from "../../../../lib/utils";
import styles from "styles/DataExplorer.module.scss";
...
/* export async function getStaticPaths() {
  const paths = (await getResourcesWithAliasList()).map(
...
export const getServerSideProps: GetStaticProps = async (context) => {
  ...
  try {
    ...
    const resourceMetadata = await Ckan.getResourceMetadata(resourceId as string);
    if (!resourceMetadata) {
    ...
    //  Only matters for tabular resources
    const resourceInfo = await Ckan.getResourceInfo(resourceId as string);
    const alias =
      (await Ckan.getResourcesWithAliasList()).find(
        (item) => item.alias_of === resourceId
      )?.name || null;

    return {
      props: { resource: { ...resourceInfo, ...resourceMetadata, alias } },
    };
  } catch (e) {
    console.log(e);
    return {
...
export default function ResourcePage({
  ...
  const apiUri = getConfig().publicRuntimeConfig.DATA_API_URL + "graphql/";
            ...
            <div className="custom-container bg-[#fcfcfc] lg:px-4 py-8 rounded">
              ...
              <div className="lg:px-8">
                {resourceFormat == "csv"
                  ? (resource.alias &&
                    resource.schema &&
                    Object.keys(resource.schema).length !== 0 && (
                      <main
                        className={
                          styles.dataExplorerTable + " custom-container"
                        }
                      >
                        <ApolloProvider client={client}>
                          <Explorer
                            dataset={resource.alias}
                            schema={convertFieldSchema(resource.schema)}
                            apiUri={
                ...
                {["geojson"].includes(resourceFormat) && (
                  <GeojsonViewer
                    layers={[
                      {
                        name: resource.name,
